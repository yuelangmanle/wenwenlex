...
- 最后更新日期：`2026-03-19`
- 当前正式版本：`1.1`
- 当前总体状态：`v1.1 已发布，v1.2 发音模块开发中`

## 2. 当前版本快照

- 项目名称：`文文Lex`
- 发布页面：[GitHub Releases](https://github.com/yuelangmanle/wenwenlex/releases)
- 当前正式版本页面：[v1.1](https://github.com/yuelangmanle/wenwenlex/releases/tag/v1.1)
...
| 发音与朗读 v1.2 | 开发中 | 已落设计、实施计划、数据库 v3 结构、发音设置入口、词典音频缓存、语音包下载骨架与系统 TTS / 桥接包兜底 |
...
- 正式版本 `1.1` 已于 `2026-03-19` 完成代码与发版准备
...
- `1.2` 主线：发音模块、词典音频缓存、系统 TTS 兜底与离线语音包挂载
- `1.2` 候选：学习数据图表和 HTML 可视化面板
- `1.2` 候选：AI 计划历史回溯、计划对比和解释性增强
- `1.2` 候选：导入表格纠错和字段补全过程进一步自动化
...
- 已确认发音模块采用“词典音频优先 + 本地缓存 + 离线语音包 + 系统 TTS 兜底”的路线
- 已写入 `v1.2` 设计文档与实施计划
- 已开始落数据库 `v3`、发音设置入口、单词详情 / 学习页播放入口、词典音频缓存与系统 TTS 调度骨架
- 已补内置语音包清单、语音包下载 worker、安装目录挂载、删除与设置页状态刷新
- 当前主要未完成项：真正的离线语音包本地推理引擎接入、完整编译验证与正式 `1.2` 发版
- 暂未发版
