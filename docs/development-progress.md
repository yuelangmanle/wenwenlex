...
- 最后更新日期：`2026-03-20`
- 当前正式版本：`1.2`
- 当前总体状态：`v1.2 已发布；v1.3 离线单词真实发音核心链路与 native 语音包正式分发链路已在开发分支验证通过`
...
| 发音与朗读 v1.3 | 开发中（核心链路与分发链路已打通） | 已完成 native metadata 合并、Sherpa ONNX runtime 脚手架、本地生成缓存、原生离线单词合成、安装校验、Release 资产打包与设置页错误可见化，待正式发版 |
...
- `1.3` 主线：真正的本地离线语音推理包、更完整的在线词典音频源，以及 scaffold pack 到真实模型资产的切换
...
- 对应云端验证已通过：`23290916401`、`23299278760`、`23299605029`
