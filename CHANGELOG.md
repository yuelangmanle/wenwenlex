...
- 新增导入诊断与修复闭环，导入 `.xlsx` 后会识别轻微问题与高风险结构问题，并支持显式触发 AI 修复后重建预览
- 新增统一发音源中心第一版，覆盖本地原生 source、MiMo 云端 TTS source、词典音频来源和默认 source 详情页
- 新增发音任务中心，支持单词级生成、整本词书后台生成、分批生成，以及对失败项进行真实重新排队
- 新增 App 内完整更新日志，版本说明直接从 `CHANGELOG.md` 生成 `release-notes.json` 并在应用内展示
- 新增离线缓存管理页，支持按缓存桶查看本地音频占用，并按桶清理或一键清空缓存
- 新增 MiMo TTS 详情能力，支持 API 检测、预设切换与云端缓存音频落地到本地生成缓存

### Changed

- 发音缓存从单一统计口径升级为多来源缓存桶，缓存总量会同时覆盖词典缓存、生成缓存和其他本地音频资产
- 低内存语音包下载链路已支持更细粒度的失败原因透出，并继续沿用分块下载与断点续传策略
- 本地原生 source 与云端 TTS source 现在都可预生成缓存并共存，播放链路会按 source / accent / cache bucket 精确命中
- 默认构建版本和 GitHub `Android CI` / `Android Release` 工作流基线同步切到 `1.8`

### Fixed

- 修复发音任务页“重试失败项”只改数据库状态、不真正重新入队的问题，当前会通过后台任务协调器重新排队
- 修复 `BookDetailViewModelTest` 与 `BookImportViewModelTest` 缺少 Robolectric runner 导致的 JVM 单测失败
- 修复语音包安装阶段 `payload checksum` 错误提示被过度泛化的问题，当前会保留更具体的失败信息

### Notes

- 本地 `:app:testDebugUnitTest` 已通过，覆盖统一发音源中心、导入诊断/修复和旧回归用例
- `v1.8` 的云端 `Android CI`、`Android Release` 与正式 Release 资产核对记录见 `docs/release-v1.8-import-pronunciation-smoke.md`

## [1.7] - 2026-03-22

### Added

- 新增动态复习优先级与三段式今日队列，结合遗忘风险、连续错误、最近反应时和跳过状态来调整高风险词、新词与复习词排序
- 新增目标设置页，支持日 / 周 / 阶段目标管理，并在首页、“我的”页和学习统计页投影本周推进、阶段完成度与连续学习状态
- 新增升级安全与诊断中心第一版，启动时会执行完整性校验，在“我的”页提供入口，并支持导出 `diagnostics.json + summary.txt` 的 zip 诊断包
- 新增 `scripts/validate_release_baseline.sh`，对版本号、`CHANGELOG.md`、`README.md`、进度书、协作书和 smoke 文档做基线校验

### Changed

- Room 已升级到 `v7`，补入学习信号字段、目标进度 JSON 和升级健康摘要字段
- 本地备份已升级到 `v6`，覆盖新的学习信号、目标设置与升级健康摘要，并保持对 `v1` 到 `v5` 旧备份的兼容恢复
- `Android CI` 与 `Android Release` 工作流现在会先执行发版基线校验，再继续跑测试、构建与正式发布

### Fixed

- 修复学习页把 `averageResponseLatencyMs` 伪造为单次时延的问题，动态复习现在优先使用真实 `lastResponseLatencyMs`
- 修复“最后一张被跳过”时的轮次结束语义，避免把“本轮暂时结束”误显示成“任务已全部完成”
