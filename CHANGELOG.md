...
- Room 数据库主版本统一提升到 `v9`，并补齐从历史 `v5`、`v6`、`v7`、`v8` 迁移到当前结构的兼容路径
- 首页、词书、学习、我的 四个主入口在数据库加载异常时统一改为中文可读错误提示，不再把底层异常直接暴露给用户
- 学习页 instrumentation 测试状态已与当前真实默认加载状态对齐，避免云端验证出现假失败

### Fixed

- 修复历史安装用户升级后触发 `A migration from 8 to 5 was required but not found` 的严重数据库兼容问题
- 修复首页和词书页因数据库迁移失败长期停在异常加载态的问题
- 修复学习页与“我的”页在数据库加载失败时直接闪退的问题
- 修复云端 CI 中数据库兼容修复后的学习页 instrumentation 测试误报失败问题

## [1.9] - 2026-03-24

### Added

- 学习流已按真实学习记录驱动首页三个入口，不再把空任务错误回退到整本词书
- 新增统一音频缓存管理页，可按来源查看缓存占用并执行清理与限额淘汰
- 新增音频任务中心，支持在线词典缓存、云端 TTS 缓存、本地离线生成三类后台任务
- 新增云端 TTS 播放链路与设置页，当前已接入 MiMo provider、默认 preset 和记忆化 API 检测
- 新增语音包断点续传下载器，支持 `.part` 临时文件续传、下载取消和失败分类
- 新增语音包多下载源能力，manifest 已支持 `downloadUrls`，设置页可切换备用源重试

### Changed

- 发音调度器现已升级为“词典缓存 -> 本地 native 生成缓存 -> native 离线合成 -> 在线词典音频 -> 云端 TTS 缓存 / 实时云端 TTS -> 系统 TTS”
- 发音设置页现已同时展示语音包资源提示、当前下载源、取消下载入口和切换下载源重试入口
- 语音包 catalog 现在同时保留 `latest/download` 和固定版本 `v1.3` 的 GitHub Release 下载地址，降低单一路径失效带来的发包风险

### Fixed

- 修复语音包下载中无法取消、失败后只能重复点下载而不能切换源的问题
- 修复语音包大文件重复全量下载、下载中断后不能从已有临时包续传的问题
- 修复云端 TTS 设置虽已存在但还未真正接入单词发音回退链路的问题
